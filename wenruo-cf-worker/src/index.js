...
// 解忧杂货铺 · 心事信笺邮件
async function sendLetterEmail(env, entry) {
  const apiKey = env.RESEND_API_KEY || env.SENDGRID_API_KEY || '';
  const to = env.GENERAL_STORE_EMAIL || env.ADMIN_EMAIL || '';
  if (!apiKey || !to) return;

  const subject = `✉️ [解忧杂货铺] 新的心事来信：${entry.nickname}`;
  const text = `称呼：${entry.nickname}\n回信邮箱：${entry.contact || '未提供'}\n时间：${entry.time}\n\n心事内容：\n${entry.message}\n\n—— 来自 温若小站 · 解忧杂货铺`;
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/general-store') {
      if (request.method === 'POST') {
        const body = await readJson(request);
        ...
        if (contact && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact)) {
          return error('Invalid contact email', 400);
        }

        const entry = {
          id: Date.now().toString(36) + Math.random().toString(36).slice(2, 6),
          nickname: nickname.slice(0, 20) || '匿名',
        ...
        // 存 KV（最近 100 封）
        const raw = await kv.get('general_store');
        let letters = [];
        if (raw) {
          try { letters = JSON.parse(raw); } catch {}
        }
        letters.unshift(entry);
        if (letters.length > 100) letters.length = 100;
        await kv.put('general_store', JSON.stringify(letters));

        // 发送邮件到店长邮箱
        if (env.ADMIN_EMAIL || env.RESEND_API_KEY || env.SENDGRID_API_KEY) {
          ctx.waitUntil(sendLetterEmail(env, entry));
        }
        return json({ success: true, entry });
      }
